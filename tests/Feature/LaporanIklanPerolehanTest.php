...
class LaporanIklanPerolehanTest extends TestCase
{
    use RefreshDatabase;
     ...
     */
     ...
     public function test_get_data_iklan_perolehan()
     {
        $permohonan = [
            [
                'id' => '5',
        ...
        foreach($permohonan as $permohonan2){
            PermohonanNomborPerolehan::create($permohonan2);
        }
        foreach($iklan as $iklan2){
            IklanPerolehan::create($iklan2);
        }
        $response = IklanPerolehan::where('id', '5')->first();
