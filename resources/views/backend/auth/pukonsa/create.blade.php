@inject('model', '\Modules\Sisdant\Models\KelasPukonsa')

@extends('backend.layouts.app')

@section('title', __('Tambah Kelas Pukonsa'))

@section('content')
    <x-forms.post :action="route('admin.auth.pukonsa.store')">
        <x-backend.card>
            <x-slot name="header">
                <span style="font-weight: bold;"> @lang('Tambah Kelas Pukonsa') </span>
            </x-slot>
            ...
            <x-slot name="body">
                <div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            <input type="text" name="tajuk" class="form-control" onkeyup="this.value = this.value.toUpperCase();" placeholder="{{ __('TAJUK') }}" value="{{ old('tajuk') }}" maxlength="255" required />
                        </div>
                    </div><!--form-group-->
