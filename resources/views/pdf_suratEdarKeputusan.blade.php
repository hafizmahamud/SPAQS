...
<body>
    ...
    <div>
    Pengarah <br>
    @foreach($alamat_negeri as $alamat_negeri)
        {!! $alamat_negeri !!}<br>
    @endforeach
    </div>
    <div>{!! $title !!},</div>
    <br>
    <b>KEPUTUSAN LEMBAGA PEROLEHAN {!! $kementerian !!} BIL.{!! $bil !!}</b>
    <hr><br>
    <div>Dengan hormatnya perkara di atas adalah dirujuk.</div>
    <div style="text-align: justify;">{!! $text_1 !!} {!! $bil !!} {!! $text_2 !!} </div>
    <div><b>{!! $tajuk_perolehan !!} ({!! $no_perolehan !!})</b></div>
