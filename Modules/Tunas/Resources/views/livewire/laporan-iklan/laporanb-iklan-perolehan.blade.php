...
<td>{{ $loop->index + 1}}</td>
<td>
    {{$row->mohonNoPerolehan->negeri['negeri']}} <br>
    {{$row->mohonNoPerolehan->section['bahagian']}}
</td>
<td>
...
<td>{{$row->kehadiranLawatanTapak->count()}}</td>
<td>{{$row->borangDaftarMinat->count()}}</td>
<td>{{$row->borangDaftarMinatBerjaya->count()}}</td>
<td>{{$row->jadualHarga->count()}}</td>
