...
class LaporanIklanFiltered extends DataTableComponent
{
    ...
    public $jenisperolehan;

    public function mount($post)
    {
        $this->j_iklan = $post->j_iklan;
        $this->negeri = $post->negeri;
        $this->kategoriperolehan = $post->kategoriperolehan;
        $this->jenisperolehan = $post->jenisperolehan;
    ...
    public function query(): Builder
    {
        ...
        return $query
                ...
                ->when($this->kategoriperolehan, fn($query, $tags) => $query
                    ->whereHas('mohonNoPerolehan', fn($query) => $query
                            ->whereHas('matrikIklan', fn($query) => $query
                                ->where('kategori_perolehan_id', $tags))))
                ->when($this->jenisperolehan, fn($query, $tags) => $query
                    ->whereHas('mohonNoPerolehan', fn($query) => $query
                            ->whereHas('matrikIklan', fn($query) => $query
                ...
                ->when($this->status, fn($query, $tags) => $query
                    ->where('status_iklan_id', $tags))
                ->when($this->tarikhmula, fn($query, $tags) => $query
                    ->whereDate('tarikh_keluar_iklan',  '>=',$this->tarikhmula)
                    ->whereDate('tarikh_keluar_iklan',  '<=',$this->tarikhakhir));
    }
    ...
    public function export()
    {
        ...
        $columns = array('No.','Negeri','No Perolehan','Tajuk','Tarikh Iklan','Tarikh Tutup','Jenis Iklan','Kategori Perolehan','Status');
        ...
        $callback = function() use($query, $columns) {
            ...
            foreach ($query as $query2) {
                ...
                fputcsv($file, array($row['No.'], $row['Negeri'], $row['No Perolehan'], $row['Tajuk'], $row['Tarikh Iklan'], $row['Tarikh Tutup'], $row['Jenis Iklan'], $row['Kategori Perolehan'], $row['Status']));
            }
