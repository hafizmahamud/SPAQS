...
class LaporanPenilaiFiltered extends DataTableComponent
{
    ...
    public function query(): Builder
    {
        ...
        $query = User::with([
                    ...
                    // {$q->whereDate('created_at', '>=', $this->tarikhmula)->whereDate('created_at', '<=', $this->tarikhakhir); }
        return $query
                ->when($this->tarikhmula, fn($query, $tags) => $query = User::with([
                        'Penilai1' => function($q) {$q->whereDate('created_at', '<=', $this->tarikhakhir)->whereDate('tarikh_sah_laku', '>=', $this->tarikhmula); },
                        'Penilai2' => function($q) {$q->whereDate('created_at', '<=', $this->tarikhakhir)->whereDate('tarikh_sah_laku', '>=', $this->tarikhmula); },
                        'section',
                        'negeri'
                    ])
                    ->where('type','like','user'));
